...
namespace PrzetwarzanieObrazow
{
    public partial class Form1 : Form
    {
        ...
        private void RunButton_Click(object sender, EventArgs e)
        {
            ...
            var filterTasks = new List<(Func<Bitmap, Bitmap> filter, PictureBox output)>
            {
                (Filters.Negative, negative),
                (Filters.Greyscale, greyscale),
                (Filters.mirror, mirror),
                (Filters.Threshhold, threshold)
            };
            ...
            Parallel.ForEach(filterTasks, pair =>
            {
                Bitmap copy;
                lock (image)
                {
                    copy = new Bitmap(image); // Thread-safe clone
                }

                Bitmap result = pair.filter(copy);

                Invoke(() =>
                {
                    pair.output.Image = result;
                    pair.output.Refresh();
                });
